...
**`System1Capture.exe` を先に手動で起動する必要はありません。** VSTが必要に応じてバックグラウンドで自動起動します。上記のように `.vst3` フォルダーと同じ階層へ配置してください。補助アプリのウィンドウは表示されません。音声を取り込むには、DAWのオーディオエンジンとプラグインを有効にします。
